...
/**
 * Vendor portal layout — requires an authenticated Supabase user.
 * Admins also pass through this gate; their RLS lets them see more once inside.
 */
export default async function VendorLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex w-full min-h-screen flex-col bg-sage-50 dark:bg-sage-950">
      <header className="sticky top-0 z-30 border-b border-border bg-surface/90 backdrop-blur dark:bg-sage-900/90 dark:border-sage-700">
        <div className="mx-auto flex h-[var(--header-height)] max-w-6xl items-center justify-between px-4 sm:px-6">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-semibold text-sage-800 dark:text-cream-50">Vendor</span>
          </div>

          {/* Desktop nav */}
          <nav aria-label="Vendor navigation" className="hidden items-center gap-1 lg:flex">
            {NAV.map((item) => (
              <Link
                key={item.href}
                href={item.href}
            ...
            {isAdmin(user) ? <PortalLinks user={user} className="ml-2 border-l border-border pl-2 dark:border-sage-700" /> : null}
          </nav>
